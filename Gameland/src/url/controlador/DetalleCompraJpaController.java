...
public class DetalleCompraJpaController implements Serializable {
    ...
    public void create(DetalleCompra detalleCompra) {
        ...
        try {
            ...
            Compras comprasid = detalleCompra.getComprasid();
            if (comprasid != null) {
                comprasid = em.getReference(comprasid.getClass(), comprasid.getId());
                detalleCompra.setComprasid(comprasid);
            }
            ...
            if (comprasid != null) {
                comprasid.getDetalleCompraList().add(detalleCompra);
                comprasid = em.merge(comprasid);
            }
            if (productoid != null) {
    ...
    public void edit(DetalleCompra detalleCompra) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            Compras comprasidOld = persistentDetalleCompra.getComprasid();
            Compras comprasidNew = detalleCompra.getComprasid();
            Producto productoidOld = persistentDetalleCompra.getProductoid();
            Producto productoidNew = detalleCompra.getProductoid();
            if (comprasidNew != null) {
                comprasidNew = em.getReference(comprasidNew.getClass(), comprasidNew.getId());
                detalleCompra.setComprasid(comprasidNew);
            }
            ...
            if (comprasidOld != null && !comprasidOld.equals(comprasidNew)) {
                comprasidOld.getDetalleCompraList().remove(detalleCompra);
                comprasidOld = em.merge(comprasidOld);
            }
            if (comprasidNew != null && !comprasidNew.equals(comprasidOld)) {
                comprasidNew.getDetalleCompraList().add(detalleCompra);
                comprasidNew = em.merge(comprasidNew);
            }
    ...
    public void destroy(Integer id) throws NonexistentEntityException {
        ...
        try {
            ...
            Compras comprasid = detalleCompra.getComprasid();
            if (comprasid != null) {
                comprasid.getDetalleCompraList().remove(detalleCompra);
                comprasid = em.merge(comprasid);
            }
            Producto productoid = detalleCompra.getProductoid();
            if (productoid != null) {
